...
class FirebaseSightingService {
  ...
  // Koleksi khusus untuk setiap user:
  // user_sightings/{uid}/items
  ...
  // Dipanggil setelah data berhasil disimpan di SQLite
  static Future<void> uploadSighting(Sighting s) async {
    final user = _auth.currentUser;
    if (user == null) {
      // Kalau belum login, kita nggak sync — tapi SQLite tetap jalan.
      return;
    }

    final col = _userCol();

    // Kalau punya id lokal, pakai sebagai docId supaya konsisten
    final docId =
  ...
  // Opsional: hapus juga dari cloud kalau dihapus dari lokal
